...
//		}
//		model.addObject("currentGroup", currentGroup);
//		//获取当前分组的具体接口信息
...
//	}
	...
	public ModelAndView apiInfo(HttpServletRequest request) {
		...
		if(StringUtils.isNotBlank(groupIdNew)) {
			model.addObject("groupIdNew", groupIdNew);
		}else {
			model.addObject("groupIdNew", "-18438671903");
		}
		if(StringUtils.isNotBlank(nd)) {
			model.addObject("nd", nd);
		}else {
			model.addObject("nd", "-18438671903");
		}
		return model;
	}
