...
import com.sgcc.hlht.service.MerchManageService;
 ...
 */
public class AccessHLHTControl {
    //@Autowired
    ...
    private static final Log log = LogFactory.getLog(AccessHLHTControl.class);

    @Pointcut("execution(public * com.kd.openplatform.action.OpenPlatformHLHTController.baseAction(..))")
...
//        Object[] args = jp.getArgs();
        ...
        String argsStr = (String) jp.getArgs()[3];
        log.info("互连互通传过来的参数参数:"+argsStr);
        JSONObject requestParam = JSONObject.parseObject(argsStr);
        String operatorIId = requestParam.get("OperatorID") + "";
        log.info("互联互通operatorIId:"+operatorIId);
        //operatorIId查appID
        CMerchantInf cMerchantInf = null;
        try {
            cMerchantInf = merchManageService.getMerchantByNo(operatorIId);
        } catch (Exception e) {
        ...
        if (cMerchantInf != null) {
            String appId = cMerchantInf.getAppId();
            log.info("通过cMerchantInf.getAppId()方法获取的appID:::::::" + appId);
            requestParam.put("appId", appId);
            request.setAttribute("appId", appId);
        }
        requestParam.put("authToken", authorization);
...
//            log.info(pass_key + "-----" + iv_str);
            ...
            for (String key : requestParam.keySet()) {
                if (!key.equals("appId")) {
                    decryptArgs.put(key, requestParam.get(key));
                }
            }
...
//                },"result":"success","status":0},"info":"能力平台调用服务成功","status":0}
                if (data.getString("status").equals("0")) {
                    ...
                    try {
                        saveUserTokenSimple(userName, authToken);
                    } catch (Exception e) {
...
//                serviceCom.dataCom(JSON.toJSONString(decryptArgs));
        ...
        //json.put("data", JSONObject.toJSONString(json.get("data")));
        if (json.get("data") != null) {
            JSONObject jsonObj = JSONObject.parseObject(json.getString("data"));
            log.info("取出的返回值的data是----" + jsonObj);
