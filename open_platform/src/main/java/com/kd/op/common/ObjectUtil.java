            ...
            if (name.startsWith("set") && name.length() > 3 && paramTypes.length == 1) {
                ...
                if (getter != null) {
                    try {
                        ...
                        if (value != null) {
                            ...
                            if (isZteClass(paramType)) {
                                try {
                                    ...
                                    logger.error("copyPropVal get value error");
                                }
                                ...
                                try {
                                    ...
                                    value = null;
                                    logger.error("value change null");
                                }
                            }
                        ...
                        logger.error("copyPropVal IllegalArgumentException");
                        // do nothing
                    } catch (IllegalAccessException e) {
                        logger.error("copyPropVal IllegalAccessException");
                        // do nothing
                    } catch (InvocationTargetException e) {
                        logger.error("copyPropVal InvocationTargetException");
                        // do nothing
                    }
                }
        ...
        for (Object source : sourceList) {
            if (isZteClass(targetClzz)) {
                try {
                    ...
                    logger.error("makeTargetList error");
                    // do nothing
                }
