...
import com.kd.op.common.HibernateHandler;
import com.kd.op.common.ObjectUtil;
import com.kd.op.common.Pagination;
import com.kd.op.dao.BaseDao;
...
public class BaseDaoImpl<T> implements BaseDao<T> {
    ...
    @SuppressWarnings({ "unchecked", "hiding" })
    public <T> List<T> findListByHql(CharSequence queryString, Object... params) {
    ...
    public <T> List<T> findListBySql(CharSequence queryString, Object... params) {
        SQLQuery query = getSession().createSQLQuery(queryString.toString()).addEntity(entityClazz);
        if (params != null) {
        	for (int i = 0; i < params.length; ++i) {
                query.setParameter(i, params[i]);
            }
    ...
    protected Query setParameter(Query query, Map<String, Object> parameterMap) {
        ...
        Iterator iterator = parameterMap.keySet().iterator(); iterator.hasNext();) {
            ...
            if (parameterMap.get(key) != null) {
                if(parameterMap.get(key).getClass().isArray()){
...
//        }
        ...
        public DmlHelper addDayRange(String fieldName, String minTime, String maxTime) {
            ...
            if (ObjectUtil.hasText(minTime) && ObjectUtil.hasText(maxTime)) {
                try {
                    ...
                    logger.error("addDayRange error");
                }
            }
	...
	@Override
	public List<T> findByUserProperty(String name, Object value) {
		String hql = "from  " + entityClazz.getSimpleName() + " where " + name + "=?";
        return findListByHql(hql, value);
	}
	@Override
	public Map<String,Object> compare(T obj1, T Obj2){
		...
		try{
			...
			for (Field f : fs) {
				if(f.isAccessible()){
                    ...
                    if( ! equals(v1, v2) ){
                        result.put(f.getName(),String.valueOf(v2));
	...
	public void compareUpdate(Object entity){
