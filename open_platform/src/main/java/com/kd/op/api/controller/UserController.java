package com.kd.op.api.controller;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.kd.op.api.enums.RegisterRouteEnum;
import com.kd.op.common.Pagination;
import com.kd.op.util.ResourceParams;
import com.kd.op.util.StringTransUtil;
import com.kd.op.util.TaskUtil;
import com.kd.op.util.rtdb.RtdbCommon;
import com.kd.op.util.rtdb.ValidateErrorLogin;
import com.kd.op.util.sm.SM2Key;
import com.kd.op.util.sm.SM2Utils;
import org.apache.commons.lang.StringUtils;
import org.apache.log4j.Logger;
import org.hibernate.criterion.Property;
import org.jeecgframework.core.common.controller.BaseController;
import org.jeecgframework.core.common.hibernate.qbc.CriteriaQuery;
import org.jeecgframework.core.common.model.common.UploadFile;
import org.jeecgframework.core.common.model.json.AjaxJson;
import org.jeecgframework.core.common.model.json.ComboBox;
import org.jeecgframework.core.common.model.json.DataGrid;
import org.jeecgframework.core.common.model.json.ValidForm;
import org.jeecgframework.core.constant.Globals;
import org.jeecgframework.core.enums.SysThemesEnum;
import org.jeecgframework.core.util.*;
import org.jeecgframework.poi.excel.ExcelImportUtil;
import org.jeecgframework.poi.excel.entity.ExportParams;
import org.jeecgframework.poi.excel.entity.ImportParams;
import org.jeecgframework.poi.excel.entity.vo.NormalExcelConstants;
import org.jeecgframework.tag.core.easyui.TagUtil;
import org.jeecgframework.tag.vo.datatable.DataTableReturn;
import org.jeecgframework.tag.vo.datatable.DataTables;
import org.jeecgframework.web.system.manager.ClientManager;
import org.jeecgframework.web.system.pojo.base.*;
import org.jeecgframework.web.system.service.DepartService;
import org.jeecgframework.web.system.service.RoleService;
import org.jeecgframework.web.system.service.SystemService;
import org.jeecgframework.web.system.service.UserService;
import org.jeecgframework.web.system.util.Check;
import org.jeecgframework.web.system.util.CheckPassword;
import org.jeecgframework.web.system.util.RandomStr;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.util.CollectionUtils;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RequestMethod;
import org.springframework.web.bind.annotation.RequestParam;
import org.springframework.web.bind.annotation.ResponseBody;
import org.springframework.web.multipart.MultipartFile;
import org.springframework.web.multipart.MultipartHttpServletRequest;
import org.springframework.web.servlet.ModelAndView;
import sunbox.api.model.ResponseVo;
import sunbox.gateway.api.model.system.OperatorItemModel;
import sunbox.gateway.api.service.system.SysSupportApiService;

import javax.annotation.Resource;
import javax.servlet.http.Cookie;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import javax.servlet.http.HttpSession;
import java.io.IOException;
import java.io.PrintWriter;
import java.sql.Timestamp;
import java.util.*;
import java.util.regex.Pattern;


/**
 * @ClassName: UserController
 * @Description: TODO(用户管理处理类)
 */
//@Scope("prototype")
@Controller
@RequestMapping("/userController")
public class UserController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(UserController.class);
	@Autowired
	private UserService userService;
	@Autowired
	private SystemService systemService;
	@Autowired
	private DepartService departService;
	@Autowired
	private RoleService roleService;
	@Resource
	private SysSupportApiService sysSupportApiService;


	/*@Autowired
	public void setSystemService(SystemService systemService) {
		this.systemService = systemService;
	}

	@Autowired
	public void setUserService(UserService userService) {
		this.userService = userService;
	}*/


	/**
	 * 菜单列表
	 * 
	 * @param request
	 * @return
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping(params = "menu")
	public void menu(HttpServletRequest request, HttpServletResponse response) {
		SetListSort sort = new SetListSort();
		TSUser u = ResourceUtil.getSessionUserName();
		// 登陆者的权限
		Set<TSFunction> loginActionlist = new HashSet<TSFunction>();// 已有权限菜单
		List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", u.getId());
		for (TSRoleUser ru : rUsers) {
			TSRole role = ru.getTSRole();
			List<TSRoleFunction> roleFunctionList = systemService.findByProperty(TSRoleFunction.class, "TSRole.id", role.getId());
			if (roleFunctionList.size() > 0) {
				for (TSRoleFunction roleFunction : roleFunctionList) {
					TSFunction function = (TSFunction) roleFunction.getTSFunction();
					loginActionlist.add(function);
				}
			}
		}
		List<TSFunction> bigActionlist = new ArrayList<TSFunction>();// 一级权限菜单
		List<TSFunction> smailActionlist = new ArrayList<TSFunction>();// 二级权限菜单
		if (loginActionlist.size() > 0) {
			for (TSFunction function : loginActionlist) {
				if (function.getFunctionLevel() == 0) {
					bigActionlist.add(function);
				} else if (function.getFunctionLevel() == 1) {
					smailActionlist.add(function);
				}
			}
		}
		// 菜单栏排序
		Collections.sort(bigActionlist, sort);
		Collections.sort(smailActionlist, sort);
		String logString = ListtoMenu.getMenu(bigActionlist, smailActionlist);
		// request.setAttribute("loginMenu",logString);
		PrintWriter writer = null;
		try {
			writer = response.getWriter();
			writer.write(logString);
		} catch (IOException e) {
			logger.error("error:",e);
		}finally{
			if(writer!=null){
				writer.flush();
				writer.close();
			}
		}
	}


	/**
	 * 用户列表页面跳转
	 * 
	 * @return
	 *
	 */
	@RequestMapping(params="getRole")
	public ModelAndView getRoleList(HttpServletRequest req){
		ModelAndView model = new ModelAndView("/iesp/system/roleSelect");
		List<TSRole> roles = roleService.laodAll();
		model.addObject("roles", roles);
		return model;
	}
	@RequestMapping(params="editUser")
	public ModelAndView editUser(HttpServletRequest request){
		ModelAndView model = new ModelAndView("/iesp/system/editUser");
		String id = request.getParameter("id");
		TSUser user=systemService.getEntity(TSUser.class, id);
		TSDepart depart =null;
		List<TSUserOrg> userOrgList = this.systemService.findByProperty(TSUserOrg.class, "tsUser.id", id);
		if(userOrgList!=null && !userOrgList.isEmpty()){
			for(TSUserOrg uo:userOrgList) {
				depart = uo.getTsDepart();
			}
		}
        StringBuffer roleCodesBuffer = new StringBuffer();
        StringBuffer roleIdsBuffer = new StringBuffer();
        StringBuffer roleNamesBuffer = new StringBuffer();
        String roleCodes = "";
		String roleIds = "";
		String roleNames = "";
		List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id",id);
		for (TSRoleUser ru : rUsers) {
			TSRole role = ru.getTSRole();
			roleCodesBuffer.append(role.getRoleCode() + ",");
			roleIdsBuffer.append(role.getId() + ",");
			roleNamesBuffer.append(role.getRoleName() + ",");
		}
		if(!roleCodesBuffer.toString().isEmpty()) {
			roleCodes = roleCodesBuffer.toString().substring(0, roleCodesBuffer.toString().length() - 1);
		}
		if(!roleIdsBuffer.toString().isEmpty()) {
			roleIds = roleIdsBuffer.toString().substring(0, roleIdsBuffer.toString().length() - 1);
		}
		if(!roleNamesBuffer.toString().isEmpty()) {
			roleNames = roleNamesBuffer.toString().substring(0, roleNamesBuffer.toString().length() - 1);
		}
		model.addObject("depart",depart);
		model.addObject("roleCodes",roleCodes);
		model.addObject("roleIds",roleIds);
		model.addObject("roleNames",roleNames);
		model.addObject("user",user);
		return model;
	}
	@RequestMapping(params="queryUser")
	public ModelAndView queryUser(HttpServletRequest request){
		ModelAndView model = new ModelAndView("/iesp/system/queryUser");
		String id = request.getParameter("id");
		TSUser user=systemService.getEntity(TSUser.class, id);
		TSDepart depart =null;
		List<TSUserOrg> userOrgList = this.systemService.findByProperty(TSUserOrg.class, "tsUser.id", id);
		if(userOrgList!=null && !userOrgList.isEmpty()){
			for(TSUserOrg uo:userOrgList) {
				depart = uo.getTsDepart();
			}
		}
        StringBuilder roleCodesBuilder = new StringBuilder();
        StringBuilder roleIdsBuilder = new StringBuilder();
        StringBuilder roleNamesBuilder = new StringBuilder();
        String roleCodes = "";
		String roleIds = "";
		String roleNames = "";
		List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id",id);
		for (TSRoleUser ru : rUsers) {
			TSRole role = ru.getTSRole();
            roleCodesBuilder.append(role.getRoleCode() + ",");
            roleIdsBuilder.append(role.getId() + ",");
            roleNamesBuilder.append(role.getRoleName() + ",");
		}
		if(!roleCodesBuilder.toString().isEmpty()) {
			roleCodes = roleCodesBuilder.toString().substring(0, roleCodesBuilder.toString().length() - 1);
		}
		if(!roleIdsBuilder.toString().isEmpty()) {
			roleIds = roleIdsBuilder.toString().substring(0, roleIdsBuilder.toString().length() - 1);
		}
		if(!roleNamesBuilder.toString().isEmpty()) {
			roleNames = roleNamesBuilder.toString().substring(0, roleNamesBuilder.toString().length() - 1);
		}
		model.addObject("depart",depart);
		model.addObject("roleCodes",roleCodes);
		model.addObject("roleIds",roleIds);
		model.addObject("roleNames",roleNames);
		model.addObject("user",user);
		systemService.recordQueryLog("user.info", Globals.MODULE_SYSTEM, id,null);
		return model;
	}
	@RequestMapping(params="checkExist")
	@ResponseBody
	public JSONObject checkExist(HttpServletRequest request) {
		JSONObject result = new JSONObject();
		result.put("valid", true);
		try {
			//判断手机号码是否为空
			String mobilePhone = request.getParameter("mobilePhone");
			String id = request.getParameter("id");
			if(StringUtils.isNotEmpty(mobilePhone)) {
				if(!userService.checkExits(mobilePhone,id)) {
					result.put("valid", false);
				}
			}
			String name = request.getParameter("userName");
			TSUser user = userService.getByName(name);
			if(user != null ) {
				result.put("valid", false);
			}
		} catch (Exception e) {
			logger.error("error:",e);
			result.put("valid", false);
		}
		return result;
	}

	@RequestMapping(params="createUser")
	@ResponseBody
	public JSONObject createUser(TSBaseUser user,HttpServletRequest request){
		JSONObject result = new JSONObject();
		String departId = request.getParameter("departId");
		result.put("success", true);
		try{
			user.setDepartid(departId);
			userService.insertUser(user);
		}catch(Exception e){
			result.put("success", false);
			result.put("msg","发生未知错误");
			logger.error("error:",e);
		}
		return result;
	}
	@RequestMapping(params = "user")
	public String user(HttpServletRequest request) {
		// 给部门查询条件中的下拉框准备数据
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
//		return "system/user/userList";
		return "iesp/system/userList";
	}

	@RequestMapping(params="getManagerUsers")
	@ResponseBody
	public JSONObject getManagerUsers(Integer limit,Integer offset,HttpServletRequest request){
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		//处理前台发过来的参数
		String departid = request.getParameter("departid");
		String status = request.getParameter("status");
		if(departid != null && !departid.isEmpty()) {
			params.put("departid", departid);
			TSDepart depart = departService.getById(departid);
			userMap.put("组织机构", depart.getDepartname());
		}
		if(status != null && !status.isEmpty()) {
			params.put("status", Short.parseShort(status));
			TaskUtil util=new TaskUtil();
			userMap.put("用户状态", util.userState(Short.parseShort(status)));
		}
		//获取平台管理员的角色code
		String[] codes = new String[3];
		codes[0] = ResourceParams.adminCode;
		codes[1] = ResourceParams.isp_check;
		codes[2] = ResourceParams.audit;
		params.put("roleCode",codes);
		return getTsUserList(limit,offset,params,userMap);
	}


	//跳转至能力提供者页面
	@RequestMapping(params = "ispUser")
	public String ispUser(HttpServletRequest request) {
		// 给部门查询条件中的下拉框准备数据
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
//		return "system/user/userList";
		return "iesp/system/ispUserList";
	}


	//能力提供者列表页面展示
	@RequestMapping(params="getIspUsers")
	@ResponseBody
	public JSONObject getIspUsers(Integer limit,Integer offset,HttpServletRequest request){
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		//处理前台发过来的参数
		String departid = request.getParameter("departid");
		String status = request.getParameter("status");
		if(departid != null && !departid.isEmpty()) {
			params.put("departid", departid);
			TSDepart depart = departService.getById(departid);
			userMap.put("组织机构", depart.getDepartname());
		}
		if(status != null && !status.isEmpty()) {
			params.put("status", Short.parseShort(status));
			TaskUtil util=new TaskUtil();
			userMap.put("用户状态", util.userState(Short.parseShort(status)));
		}
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.ispCode;
		params.put("roleCode",codes);
		return getTsUserList(limit,offset,params,userMap);
	}


	/**
	 * 跳转能力提供者审核界面
	 * @return
	 */
	@RequestMapping(params = "approvalIspUserList")
	public String approvalIspUserList() {
		return "/iesp/system/approvalIspUserList";
	}

	/**
	 * 能力提供者待审核列表展示
	 * @param limit
	 * @param offset
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "getapprovalIspUserList")
	@ResponseBody
	public JSONObject getapprovalIspUserList(Integer limit, Integer offset, HttpServletRequest request) {
		JSONObject result = new JSONObject();
		JSONArray array = new JSONArray();
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		// 处理前台发过来的参数
		String userName = request.getParameter("userName");

		if (userName != null && !userName.isEmpty()) {
            String s = StringTransUtil.stringReplace(userName);
            params.put("userName", "%" + s + "%");
		}
		//待审核状态
		params.put("checkStatus", Globals.User_Audit_Wait);
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.ispCode;
		params.put("roleCode",codes);

			if(offset==1) {
				systemService.recordQueryLog("check.user.list", Globals.MODULE_USER, "", params);
				if(userName!=null && !userName.isEmpty()) {
					systemService.addLog("查看待审核用户列表成功,查询条件为："+userName, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
					if(Pattern.matches("[A-Za-z0-9_]+",userName)) {
						logger.debug("查看待审核用户列表成功");
					}else{
						logger.debug("待审核用户名格式错误");
					}
				}else{
					systemService.addLog("查看待审核用户列表成功", Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
				}
				logger.debug("查看待审核用户列表成功");
			}

			systemService.addLog("查看待审核用户列表失败", Globals.MODULE_USER,Globals.Log_Leavel_ERROR, Globals.FAILD);
			logger.debug("查看待审核用户列表失败");
			return getTsUserList(limit,offset,params,userMap);
	}

	/**
	 * 跳转修改能力提供者审核状态页面
	 * @param request
	 * @return
	 */
	@RequestMapping(params="editApprovalIspUser")
	public ModelAndView editApprovalIspUserView(HttpServletRequest request) {

		String id = request.getParameter("id");
		TSBaseUser baseUser = new TSBaseUser();

		baseUser = userService.getById(id);

		ModelAndView model = new ModelAndView("/iesp/system/editApprovalIspUser");
		model.addObject("baseUser", baseUser);
		return model;
	}


	/**
	 * 审核能力提供者操作
	 * @param baseUser
	 * @param request
	 * @return
	 */
	@RequestMapping(params="updateApprovalIspUser")
	@ResponseBody
	public JSONObject updateApprovalIspUser(TSBaseUser baseUser,HttpServletRequest request) {
		JSONObject result = new JSONObject();
		//用户id
		String userId = request.getParameter("baseUserId");
		//审核状态
		String checkStatus = request.getParameter("checkStatus");
		Short status = 0;
		//审核意见
		String approvalOpinion = request.getParameter("approvalOpinion");

		if(checkStatus != null && !checkStatus.isEmpty()) {
			status = Short.parseShort(checkStatus);
		}

		//log信息
		String logMessage = "";
		if ("2".equals(checkStatus)) {
			logMessage = "审核通过";
		} else if ("3".equals(checkStatus)) {
			logMessage = "审核失败";
		}

		try {
			//首先判断用户的来源是哪里
			TSUser user = userService.getById(userId);
			//车联网用户授权
			if(user.getRegisterType() != null && user.getRegisterType().equals(RegisterRouteEnum.chelianwang.getCode())){
				result = approvalClwUser(user,request);
				if((boolean)result.get("success") == false){
					return result;
				}
			}
			userService.updateApprovalByUserId(userId, status, approvalOpinion);
			result.put("success", true);
			systemService.addLog("更新审核状态成功，更新内容为：" + baseUser.getUserName() + logMessage, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
			logger.debug("更新审核状态：" + baseUser.getUserName() + " 成功");
		} catch (Exception e) {
			result.put("success", false);
			result.put("msg", "发生未知错误");
			systemService.addLog("更新审核状态：" +baseUser.getUserName()  + " 失败" + e.getMessage(), Globals.MODULE_USER, Globals.Log_Leavel_ERROR,Globals.FAILD);
			logger.error("error:",e);

		}
		return result;
	}


	//跳转至能力订阅者页面
	@RequestMapping(params = "isvUser")
	public String isvUser(HttpServletRequest request) {
		// 给部门查询条件中的下拉框准备数据
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
//		return "system/user/userList";
		return "iesp/system/isvUserList";
	}

	//能力订阅者列表页面展示
	@RequestMapping(params="getIsvUsers")
	@ResponseBody
	public JSONObject getIsvUsers(Integer limit,Integer offset,HttpServletRequest request){
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		//处理前台发过来的参数
		String departid = request.getParameter("departid");
		String status = request.getParameter("status");
		if(departid != null && !departid.isEmpty()) {
			params.put("departid", departid);
			TSDepart depart = departService.getById(departid);
			userMap.put("组织机构", depart.getDepartname());
		}
		if(status != null && !status.isEmpty()) {
			params.put("status", Short.parseShort(status));
			TaskUtil util=new TaskUtil();
			userMap.put("用户状态", util.userState(Short.parseShort(status)));
		}
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.isvCode;
		params.put("roleCode",codes);
		return getTsUserList(limit,offset,params,userMap);
	}


	/**
	 * 跳转能力订阅者审核界面
	 * @return
	 */
	@RequestMapping(params = "approvalIsvUserList")
	public String approvalIsvUserList() {
		return "/iesp/system/approvalIsvUserList";
	}

	/**
	 * 能力订阅者待审核列表展示
	 * @param limit
	 * @param offset
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "getapprovalIsvUserList")
	@ResponseBody
	public JSONObject getapprovalIsvUserList(Integer limit, Integer offset, HttpServletRequest request) {
		JSONObject result = new JSONObject();
		JSONArray array = new JSONArray();
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		// 处理前台发过来的参数
		String userName = request.getParameter("userName");

		if (userName != null && !userName.isEmpty()) {
            String s = StringTransUtil.stringReplace(userName);
            params.put("userName", "%" + s + "%");
		}
		//待审核状态
		params.put("checkStatus", Globals.User_Audit_Wait);
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.isvCode;
		params.put("roleCode",codes);

			if(offset==1) {
				systemService.recordQueryLog("check.user.list", Globals.MODULE_USER, "", params);
				if(userName!=null && !userName.isEmpty()) {
					systemService.addLog("查看待审核用户列表成功,查询条件为："+userName, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
					if(Pattern.matches("[A-Za-z0-9_]+",userName)) {
						logger.debug("查看待审核用户列表成功");
					}else{
						logger.debug("待审核用户名格式错误");
					}
				}else{
					systemService.addLog("查看待审核用户列表成功", Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
				}
				logger.debug("查看待审核用户列表成功");
			}

			systemService.addLog("查看待审核用户列表失败", Globals.MODULE_USER,Globals.Log_Leavel_ERROR, Globals.FAILD);
			logger.debug("查看待审核用户列表失败");
			return getTsUserList(limit,offset,params,userMap);
	}

	/**
	 * 跳转修改能力订阅者审核状态页面
	 * @param request
	 * @return
	 */
	@RequestMapping(params="editApprovalIsvUser")
	public ModelAndView editApprovalIsvUserView(HttpServletRequest request) {

		String id = request.getParameter("id");
		TSBaseUser baseUser = new TSBaseUser();

		baseUser = userService.getById(id);

		ModelAndView model = new ModelAndView("/iesp/system/editApprovalIsvUser");
		model.addObject("baseUser", baseUser);
		return model;
	}


	/**
	 * 审核能力订阅者操作
	 * @param baseUser
	 * @param request
	 * @return
	 */
	@RequestMapping(params="updateApprovalIsvUser")
	@ResponseBody
	public JSONObject updateApprovalIsvUser(TSBaseUser baseUser,HttpServletRequest request) {
		JSONObject result = new JSONObject();
		//用户id
		String userId = request.getParameter("baseUserId");
		//审核状态
		String checkStatus = request.getParameter("checkStatus");
		Short status = 0;
		//审核意见
		String approvalOpinion = request.getParameter("approvalOpinion");

		if(checkStatus != null && !checkStatus.isEmpty()) {
			status = Short.parseShort(checkStatus);
		}

		//log信息
		String logMessage = "";
		if ("2".equals(checkStatus)) {
			logMessage = "审核通过";
		} else if ("3".equals(checkStatus)) {
			logMessage = "审核失败";
		}

		try {
			//首先判断用户的来源是哪里
			TSUser user = userService.getById(userId);
			//车联网用户授权
			if(user.getRegisterType() != null && user.getRegisterType().equals(RegisterRouteEnum.chelianwang.getCode())){
				result = approvalClwUser(user,request);
				if((boolean)result.get("success") == false){
					return result;
				}
			}
			userService.updateApprovalByUserId(userId, status, approvalOpinion);
			result.put("success", true);
			systemService.addLog("更新审核状态成功，更新内容为：" + baseUser.getUserName() + logMessage, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
			logger.debug("更新审核状态：" + baseUser.getUserName() + " 成功");
		} catch (Exception e) {
//			e.printStackTrace();
			result.put("success", false);
			result.put("msg", "发生未知错误");
			systemService.addLog("更新审核状态：" +baseUser.getUserName()  + " 失败" + e.getMessage(), Globals.MODULE_USER, Globals.Log_Leavel_ERROR,Globals.FAILD);
			logger.error("更新审核状态：" + baseUser.getUserName()  + " 失败," + e.getMessage());

		}
		return result;
	}

	/**
	 * 给车联网用户加标签
	 * @param user
	 */
	private JSONObject approvalClwUser(TSUser user,HttpServletRequest request){
		JSONObject result = new JSONObject();
		//获取accessToken
		String accessToken = RtdbCommon.loadAccessToken();
		if(accessToken == null || accessToken.isEmpty()){
			result.put("msg","获取车联网的连接参数失败！");
			result.put("success",false);
			return result;
		}
		logger.info("accessToken:" + accessToken);
		//给车联网账号进行添加标签
		Client client = ClientManager.getInstance().getClient(request.getSession().getId());
		if (client != null) {
			logger.info("授权operator的参数：" + client.getToken());
		}
		if(client == null || client.getToken().isEmpty()){
			result.put("msg","获取车联网的连接参数失败！");
			result.put("success", false);
			return result;
		}
		OperatorItemModel operator = new OperatorItemModel();
		operator.setBusinessLabels(ResourceParams.businessLabel + "");
		operator.setAccessToken(accessToken);
		operator.setId(Integer.parseInt(user.getThirdId()));
		operator.setToken(client.getToken());

		ResponseVo vo = sysSupportApiService.authOperatorBiz(operator);
		if(vo.getStatus() != 0){
			result.put("msg","车联网账号授权标签失败！");
			result.put("success", false);
			return result;
		}
		result.put("success",true);
		return result;
	}


	//跳转至应用订购者页面
	@RequestMapping(params = "appBuyer")
	public String appBuyer(HttpServletRequest request) {
		// 给部门查询条件中的下拉框准备数据
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		request.setAttribute("departsReplace", RoletoJson.listToReplaceStr(departList, "departname", "id"));
		departList.clear();
//		return "system/user/userList";
		return "iesp/system/appBuyerList";
	}

	//应用订购者列表页面展示
	@RequestMapping(params="getAppBuyer")
	@ResponseBody
	public JSONObject getAppBuyer(Integer limit,Integer offset,HttpServletRequest request){
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		//处理前台发过来的参数
		String departid = request.getParameter("departid");
		String status = request.getParameter("status");
		if(departid != null && !departid.isEmpty()) {
			params.put("departid", departid);
			TSDepart depart = departService.getById(departid);
			userMap.put("组织机构", depart.getDepartname());
		}
		if(status != null && !status.isEmpty()) {
			params.put("status", Short.parseShort(status));
			TaskUtil util=new TaskUtil();
			userMap.put("用户状态", util.userState(Short.parseShort(status)));
		}
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.appBuyer;
		params.put("roleCode",codes);
		return getTsUserList(limit,offset,params,userMap);
	}


	/**
	 * 跳转应用订购者审核界面
	 * @return
	 */
	@RequestMapping(params = "approvalAppBuyerUserList")
	public String approvalAppBuyerUserList() {
		return "/iesp/system/approvalAppBuyerUserList";
	}

	/**
	 * 应用订购者待审核列表展示
	 * @param limit
	 * @param offset
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "getapprovalAppBuyerUserList")
	@ResponseBody
	public JSONObject getapprovalAppBuyerUserList(Integer limit, Integer offset, HttpServletRequest request) {
		JSONObject result = new JSONObject();
		JSONArray array = new JSONArray();
		Map<String, Object> params = new HashMap<>();
		Map<String,Object> userMap=new HashMap<String,Object>();
		// 处理前台发过来的参数
		String userName = request.getParameter("userName");

		if (userName != null && !userName.isEmpty()) {
            String s = StringTransUtil.stringReplace(userName);
            params.put("userName", "%" + s + "%");
		}
		//待审核状态
		params.put("checkStatus", Globals.User_Audit_Wait);
		//获取平台管理员的角色code
		String[] codes = new String[1];
		codes[0] = ResourceParams.appBuyer;
		params.put("roleCode",codes);

			if(offset==1) {
				systemService.recordQueryLog("check.user.list", Globals.MODULE_USER, "", params);
				if(userName!=null && !userName.isEmpty()) {
					systemService.addLog("查看待审核用户列表成功,查询条件为："+userName, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
					if(Pattern.matches("[A-Za-z0-9_]+",userName)) {
						logger.debug("查看待审核用户列表成功");
					}else{
						logger.debug("待审核用户名格式错误");
					}
				}else{
					systemService.addLog("查看待审核用户列表成功", Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
				}
				logger.debug("查看待审核用户列表成功");
			}

			systemService.addLog("查看待审核用户列表失败", Globals.MODULE_USER,Globals.Log_Leavel_ERROR, Globals.FAILD);
			logger.debug("查看待审核用户列表失败");
			return getTsUserList(limit,offset,params,userMap);
	}

	/**
	 * 跳转修改用户审核状态页面
	 * @param request
	 * @return
	 */
	@RequestMapping(params="editApprovalAppBuyerUser")
	public ModelAndView editApprovalAppBuyerUserView(HttpServletRequest request) {

		String id = request.getParameter("id");
		TSBaseUser baseUser = new TSBaseUser();

		baseUser = userService.getById(id);

		ModelAndView model = new ModelAndView("/iesp/system/editApprovalAppBuyerUser");
		model.addObject("baseUser", baseUser);
		return model;
	}


	/**
	 * 审核用户操作
	 * @param baseUser
	 * @param request
	 * @return
	 */
	@RequestMapping(params="updateApprovalAppBuyerUser")
	@ResponseBody
	public JSONObject updateApprovalAppBuyerUser(TSBaseUser baseUser,HttpServletRequest request) {
		JSONObject result = new JSONObject();
		//用户id
		String userId = request.getParameter("baseUserId");
		//审核状态
		String checkStatus = request.getParameter("checkStatus");
		Short status = 0;
		//审核意见
		String approvalOpinion = request.getParameter("approvalOpinion");

		if(checkStatus != null && !checkStatus.isEmpty()) {
			status = Short.parseShort(checkStatus);
		}

		//log信息
		String logMessage = "";
		if ("2".equals(checkStatus)) {
			logMessage = "审核通过";
		} else if ("3".equals(checkStatus)) {
			logMessage = "审核失败";
		}

		try {
			userService.updateApprovalByUserId(userId, status, approvalOpinion);
			result.put("success", true);
			systemService.addLog("更新审核状态成功，更新内容为：" + baseUser.getUserName() + logMessage, Globals.MODULE_USER, Globals.Log_Leavel_INFO,Globals.SUCCESS);
			logger.debug("更新审核状态：" + baseUser.getUserName() + " 成功");
		} catch (Exception e) {
			result.put("success", false);
			result.put("msg", "发生未知错误");
			systemService.addLog("更新审核状态：" +baseUser.getUserName()  + " 失败" + e.getMessage(), Globals.MODULE_USER, Globals.Log_Leavel_ERROR,Globals.FAILD);
			logger.error("更新审核状态：" + baseUser.getUserName()  + " 失败," + e.getMessage());

		}
		return result;
	}



	public JSONObject getTsUserList(Integer limit,Integer offset,Map<String, Object> params,Map<String,Object> userMap){
		JSONObject result = new JSONObject();
		JSONArray array = new JSONArray();
		try{
			Pagination<TSUser> pagination = userService.getUserList(limit, offset, params);
			for(TSUser user:pagination.getItems()) {
				JSONObject obj = new JSONObject();
				obj.put("id", user.getId());
				obj.put("userName", user.getUserName());
				obj.put("realName", user.getRealName());
				//TSDepart depart = departService.getById(user.getDepartid());
				//obj.put("departName", depart.getDepartname());
//				if(user.getDepartid()!=null && !user.getDepartid().isEmpty()){
//					TSDepart depart = departService.getById(user.getDepartid());
//					obj.put("departName", depart.getDepartname());
//				}
				obj.put("checkStatus", user.getCheckStatus());
				List<TSRoleUser> rus = systemService.findByProperty(TSRoleUser.class,"TSUser.id",user.getId());
				List<String> roleNames = new ArrayList<>();
				if(rus != null){
					for(TSRoleUser ru:rus){
						if(ru.getTSRole() != null){
							roleNames.add(ru.getTSRole().getRoleName());
						}
					}
				}
				obj.put("roleName", StringUtils.join(roleNames,","));
				obj.put("createDate", user.getCreateDate());
				obj.put("status", user.getStatus());
				obj.put("type", user.getType());
				obj.put("registerType",user.getRegisterType());
				array.add(obj);
			}
			if(offset==1){
				//判断是否记录日志
				systemService.recordQueryLog("user.list", Globals.MODULE_SYSTEM, "",userMap);
				logger.debug("查看用户列表列表信息成功");
			}
			result.put("total", pagination.getRowsCount());
			result.put("rows", array);
			return result;
		}catch(Exception e){
			systemService.addLog("查看用户列表信息失败", Globals.MODULE_SYSTEM, Globals.Log_Leavel_ERROR,"0");
			logger.debug("查看用户列表信息失败");
			logger.error("执行失败：" + e.getMessage());
			return null;
		}
	}
	/**
	 * 用户信息
	 * 
	 * @return
	 */
	@RequestMapping(params = "userinfo")
	public String userinfo(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUserName();
		request.setAttribute("user", user);
		return "system/user/userinfo";
	}

	@RequestMapping(params = "showUserInfo")
	public ModelAndView showUserInfo(HttpServletRequest request) {
		ModelAndView model = new ModelAndView("/iesp/system/userInfo");
		TSUser user = ResourceUtil.getSessionUserName();
		model.addObject("user",user);
        StringBuffer rolesBuffer = new StringBuffer();
        String roles = "";
        StringBuffer departsBuffer = new StringBuffer();
        String departs = "";
		List<TSUserOrg> userOrgList = this.systemService.findByProperty(TSUserOrg.class, "tsUser.id", user.getId());
		for(TSUserOrg uo:userOrgList) {
			TSDepart depart = uo.getTsDepart();
            departsBuffer.append(depart.getDepartname() + ",");
		}
		if(!departsBuffer.toString().isEmpty()) {
			departs = departsBuffer.toString().substring(0, departsBuffer.toString().length() - 1);
		}
		List<TSRoleUser> rUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		for (TSRoleUser ru : rUsers) {
			TSRole role = ru.getTSRole();
            rolesBuffer.append(role.getRoleName() + ",");
		}
		if(!rolesBuffer.toString().isEmpty()) {
			roles = rolesBuffer.toString().substring(0, rolesBuffer.toString().length() - 1);
		}
		model.addObject("departs",departs);
		model.addObject("roles",roles);
		return model;
	}

	/**
	 * 修改密码
	 * 
	 * @return
	 */
	@RequestMapping(params = "changepassword")
	public String changepassword(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUserName();
		request.setAttribute("user", user);
		return "system/user/changepassword";
	}



	/**
	 * 修改密码
	 * 
	 * @return
	 */
	@RequestMapping(params = "savenewpwd")
	@ResponseBody
	public AjaxJson savenewpwd(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		TSUser user = ResourceUtil.getSessionUserName();
		HttpSession session = request.getSession();
		String randomStr=oConvertUtils.getString(request.getParameter("randomStr"));
		Client client = ClientManager.getInstance().getClient();
		String str = client.getRandomStr();
		//验证会话重放攻击
		if(!randomStr.equals(str)){
			session.removeAttribute(randomStr);
			j.setMsg("此次操作存在安全隐患");
			j.setSuccess(false);
			systemService.addLoginFaildLog("用户"+user.getUserName()+"修改密码存在安全隐患！",
					Globals.Log_Type_LOGIN, Globals.Log_Leavel_INFO, "0",user.getUserName());
			return j;
		}else {
			client.setRandomStr(RandomStr.createRandomString(15));
		}
		//解析加密后的密码
		//获取密钥
		String oldPassword = oConvertUtils.getString(request.getParameter("oldpass"));
		String newPassword = oConvertUtils.getString(request.getParameter("newpass"));
		String newpass="";//待解密新密码
		String decrypted="";//待解密数据库取出的旧密码
		String oldpass="";//待解密旧密码
		String hispass="";//待解密历史密码
		String sm3newpass="";//待解密新密码
		String sm3decrypted="";//待解密数据库取出的旧密码
		String sm3oldpass="";//待解密旧密码
		String sm3hispass="";//待解密历史密码
		Integer newpassLength;//判断新密码长度
		try {
			newpass = SM2Utils.decrypt(SM2Key.DBPrivKey, newPassword);
			String smnewpass[]=SM2Utils.sm3decrypt(newpass);
			sm3newpass=smnewpass[0];
			oldpass=SM2Utils.decrypt(SM2Key.DBPrivKey, oldPassword);
			String[] smoldpass = SM2Utils.sm3decrypt(oldpass);
			sm3oldpass=smoldpass[0];
			decrypted = SM2Utils.decrypt(SM2Key.DBPrivKey, user.getPassword());
			String[] smdecry = SM2Utils.sm3decrypt(decrypted);
			sm3decrypted=smdecry[0];
			if(user.getHispassword()!=null && !user.getHispassword().isEmpty()){
				hispass = SM2Utils.decrypt(SM2Key.DBPrivKey, user.getHispassword());
				String[] smhispass = SM2Utils.sm3decrypt(hispass);
				sm3hispass=smhispass[0];
			}
			newpassLength=newpass.length();
		} catch (Exception e1) {
			j.setMsg("修改密码失败，请重新输入");
			j.setSuccess(false);
			return j;
		}
		if(!sm3decrypted.equals(sm3oldpass)){
			j.setMsg("原密码不正确");
			j.setSuccess(false);
			//添加日志
			systemService.addLog(user.getUserName() + "重置自己的密码失败，原密码错误", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO, "1");
			return j;
		}
		if(sm3newpass.equals(user.getUserName())){
			j.setMsg("密码不能与用户名相同，请重新输入！");
			j.setSuccess(false);
			return j;
		}
		if(sm3newpass.contains(user.getUserName())){
			j.setMsg("密码不能包含用户名，请重新输入！");
			j.setSuccess(false);
			return j;
		}
		if (newpassLength<8 || newpassLength>20) {
			j.setMsg("密码长度为8-20！");
			j.setSuccess(false);
			return j;
		}
		if (!CheckPassword.checkContent(sm3newpass)){
			j.setMsg("密码必须包含字母数字和特定范围的特殊字符！");
			j.setSuccess(false);
			return j;
		}
		if(sm3newpass.equals(sm3hispass)){
			j.setMsg("新密码不能与历史密码相同");
			j.setSuccess(false);
			return j;
		}
		if(sm3newpass.equals(sm3oldpass)){
			j.setMsg("新密码不能与原密码相同");
			j.setSuccess(false);
		}else {
			try {
				user.setPassword(newPassword);
				user.setHispassword(oldPassword);
			} catch (Exception e) {
				logger.error("执行失败：" + e.getMessage());
			}
			user.setPass_update_time(new Timestamp(new Date().getTime()));
			user.setPass_update_flag((short)1);
			systemService.updateEntitie(user);
			//修改密码也在密码校验表中更新用户的密码
			SysPassValidate passValidate = systemService.getEntity(SysPassValidate.class, user.getId());
			if (passValidate == null) {
				passValidate = new SysPassValidate();
				passValidate.setPassword(user.getPassword());
				passValidate.setUpdateTime(new Date());
				passValidate.setUserId(user.getId());
				passValidate.setUserName(user.getUserName());
			}else{
				passValidate.setPassword(user.getPassword());
				passValidate.setUpdateTime(new Date());
			}
			systemService.save(passValidate);
			j.setMsg("修改成功，请重新登录!");
			//添加日志
			systemService.addLog(user.getUserName() + "重置自己的密码成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO, "1");
		}
		return j;
	}


	/**
	 * 
	 * 修改用户密码
	 * @author Chj
	 */

	@RequestMapping(params = "changepasswordforuser")
	public ModelAndView changepasswordforuser(String id, HttpServletRequest req) {
		req.setAttribute("id", id);
		return new ModelAndView("iesp/system/adminchangepwd");
	}
	@RequestMapping("updatePassword")
	public ModelAndView updatePassword(HttpServletRequest req){
		ModelAndView model=new ModelAndView("iesp/system/updatePassword");
		String id = req.getParameter("id");
		model.addObject("id", id);
		return model;
	}
	@RequestMapping(params="resetPassword",method=RequestMethod.POST)
	@ResponseBody
	public JSONObject resetPassword(HttpServletRequest req){
		JSONObject result=new JSONObject();
		String id = req.getParameter("id");
		String password = req.getParameter("password");
		result.put("success", true);
		try{
			userService.resetPassword(id, password);
		}catch(Exception e){
			result.put("success", false);
			result.put("msg", "发生未知错误");
			logger.error("执行失败：" + e.getMessage());
		}
		return result;
	}

	/**
	 * 重置密码
	 * @param req
	 * @return
	 */
	@RequestMapping(params = "savenewpwdforuser")
	@ResponseBody
	public JSONObject savenewpwdforuser(HttpServletRequest req) {
		String message = null;
		JSONObject result=new JSONObject();
		HttpSession session = req.getSession();
		String randomStr=oConvertUtils.getString(req.getParameter("randomStr"));
		Client client = ClientManager.getInstance().getClient();
		String str = client.getRandomStr();
		//验证会话重放攻击
		if(!randomStr.equals(str)){
			session.removeAttribute(randomStr);
			result.put("msg","此次操作存在安全隐患！");
			result.put("success",false);
			systemService.addLoginFaildLog("重置密码存在安全隐患！",
					Globals.Log_Type_LOGIN, Globals.Log_Leavel_INFO, "0",null);
			return result;
		}else {
			client.setRandomStr(RandomStr.createRandomString(15));
		}
		//获取密钥
		String id = oConvertUtils.getString(req.getParameter("id"));
		String password = oConvertUtils.getString(req.getParameter("password"));
		TSUser user=null;
		if(id!=null){
			user= systemService.getEntity(TSUser.class,id);
		}
		result.put("success",true);
		String newpass="";
		String oldpass="";
		String hispass="";
//		String sm3newpass="";
//		String sm3oldpass="";
//		String sm3hispass="";
		try{
			newpass = SM2Utils.decrypt(SM2Key.DBPrivKey, password);
			String[] smnewpass = SM2Utils.sm3decrypt(newpass);
//			sm3newpass=smnewpass[0];
			if(user!=null){
				oldpass = SM2Utils.decrypt(SM2Key.DBPrivKey, user.getPassword());//需要修改
				hispass = SM2Utils.decrypt(SM2Key.DBPrivKey, user.getHispassword());
			}

			String[] smoldpass = SM2Utils.sm3decrypt(oldpass);
//			sm3oldpass=smoldpass[0];
			String[] smhispass = SM2Utils.sm3decrypt(hispass);
//			sm3hispass=smhispass[0];
		}catch(Exception e){
			logger.error("执行失败：" + e.getMessage());
		}
		if(newpass == null){
			result.put("msg","密码解析失败!");
			result.put("success",false);
			return result;
		}
		if(user!=null){
			if(newpass.equals(user.getUserName())){
				result.put("msg","密码不能与用户名相同，请重新输入！");
				result.put("success",false);
				return result;
			}
			if(newpass.contains(user.getUserName())){
				result.put("msg","密码不能包含用户名，请重新输入！");
				result.put("success",false);
				return result;
			}
		}
		if(newpass.length()<8 && newpass.length()>20) {
			result.put("msg","密码长度为8-20位！");
			result.put("success",false);
			return result;
		}
		if (!CheckPassword.checkContent(newpass)) {
			result.put("msg","密码必须包含字母数字和特定范围的特殊字符！");
			result.put("success",false);
			return result;
		}
		if(newpass.equals(oldpass)){
			result.put("msg","新密码不能与原密码相同");
			result.put("success",false);
			return result;
		}
		if(newpass.equals(hispass)){
			result.put("msg","新密码不能与历史密码相同");
			result.put("success",false);
			return result;
		}
		if (user != null) {
			user.setHispassword(user.getPassword());
			user.setPassword(password);
			user.setStatus(Globals.User_Normal);
			user.setActivitiSync(user.getActivitiSync());
			user.setPass_update_time(new Timestamp(new Date().getTime()));
			systemService.updateEntitie(user);
			message = "用户: " + user.getUserName() + "密码重置成功";
			systemService.addLog(message, Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO, "1");
			//修改密码也在密码校验表中更新用户的密码
			SysPassValidate passValidate = systemService.getEntity(SysPassValidate.class, user.getId());
			if (passValidate == null) {
				passValidate = new SysPassValidate();
				passValidate.setPassword(password);
				passValidate.setUpdateTime(new Date());
				passValidate.setUserId(user.getId());
				passValidate.setUserName(user.getUserName());
			} else {
				passValidate.setPassword(password);
				passValidate.setUpdateTime(new Date());
			}
			systemService.save(passValidate);
		}
		return result;
	}
	/**
	 * 解锁用户
	 * 
	 * @author pu.chen
	 */
	@RequestMapping(params = "unlockUser")
	@ResponseBody
	public AjaxJson lock(String id, HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		String message = null;
		TSUser user = systemService.getEntity(TSUser.class, id);
		//		if("admin".equals(user.getUserName())){
		//			message = "超级管理员[admin]不可操作";
		//			j.setMsg(message);
		//			return j;
		//		}
		//将锁定用户状态修改为 激活
		//如果用户之前的状态为休眠，并且是临时用户，就将创建时间更新
		if(user.getStatus().equals(Globals.User_Sleep) && user.getType() .equals((short)0)) {
			user.setCreateDate(new Date());
		}
		user.setStatus(Globals.User_Normal);
		try{
			//将实时库中的缓存信息删除
			ValidateErrorLogin.clearErrorLogin(user.getUserName());
			//更新用户
			userService.updateEntitie(user);
			message = "用户：" + user.getUserName() + "激活成功!";
			systemService.addLog(message, Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
		}catch(Exception e){
			message = "激活用户操作失败!";
			systemService.addLog(message, Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"0");
		}
		j.setMsg(message);
		return j;
	}


	/**
	 * 得到角色列表
	 * 
	 * @return
	 */
	@RequestMapping(params = "role")
	@ResponseBody
	public List<ComboBox> role(HttpServletResponse response, HttpServletRequest request, ComboBox comboBox) {
		String id = request.getParameter("id");
		List<ComboBox> comboBoxs = new ArrayList<ComboBox>();
		List<TSRole> roles = new ArrayList<TSRole>();
		if (StringUtil.isNotEmpty(id)) {
			List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", id);
			if (roleUser.size() > 0) {
				for (TSRoleUser ru : roleUser) {
					roles.add(ru.getTSRole());
				}
			}
		}
		List<TSRole> roleList = systemService.getList(TSRole.class);
		comboBoxs = TagUtil.getComboBox(roleList, roles, comboBox);

		roleList.clear();
		roles.clear();

		return comboBoxs;
	}

	/**
	 * 得到部门列表
	 * 
	 * @return
	 */
	@RequestMapping(params = "depart")
	@ResponseBody
	public List<ComboBox> depart(HttpServletResponse response, HttpServletRequest request, ComboBox comboBox) {
		String id = request.getParameter("id");
		List<ComboBox> comboBoxs = new ArrayList<ComboBox>();
		List<TSDepart> departs = new ArrayList();
		if (StringUtil.isNotEmpty(id)) {
			TSUser user = systemService.get(TSUser.class, id);
			//			if (user.getTSDepart() != null) {
			//				TSDepart depart = systemService.get(TSDepart.class, user.getTSDepart().getId());
			//				departs.add(depart);
			//			}
			// todo zhanggm 获取指定用户的组织机构列表
			List<TSDepart[]> resultList = systemService.findHql("from TSDepart d,TSUserOrg uo where d.id=uo.orgId and uo.id=?", id);
			for (TSDepart[] departArr : resultList) {
				departs.add(departArr[0]);
			}
		}
		List<TSDepart> departList = systemService.getList(TSDepart.class);
		comboBoxs = TagUtil.getComboBox(departList, departs, comboBox);
		return comboBoxs;
	}

	/**
	 * easyuiAJAX用户列表请求数据 
	 * @param request
	 * @param response
	 * @param dataGrid
	 */
	@RequestMapping(params = "datagrid")
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);

		Short[] userstate = new Short[]{Globals.User_Normal, Globals.User_ADMIN, Globals.User_Forbidden};
		cq.in("status", userstate);
		cq.eq("deleteFlag", Globals.Delete_Normal);

		String orgIds = request.getParameter("orgIds");
		List<String> orgIdList = extractIdListByComma(orgIds);
		// 获取 当前组织机构的用户信息
		if (!CollectionUtils.isEmpty(orgIdList)) {
			CriteriaQuery subCq = new CriteriaQuery(TSUserOrg.class);
			subCq.setProjection(Property.forName("tsUser.id"));
			subCq.in("tsDepart.id", orgIdList.toArray());
			subCq.add();

			cq.add(Property.forName("id").in(subCq.getDetachedCriteria()));
		}


		cq.add();
		this.systemService.getDataGridReturn(cq, true);

		List<TSUser> cfeList = new ArrayList<TSUser>();
		for (Object o : dataGrid.getResults()) {
			if (o instanceof TSUser) {
				TSUser cfe = (TSUser) o;
				if (StringUtils.isNotEmpty(cfe.getId())) {
					List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", cfe.getId());
					if (roleUser.size() > 0) {
                        StringBuffer stringBuffer = new StringBuffer();
                        String roleName = "";
						for (TSRoleUser ru : roleUser) {
                            stringBuffer.append(ru.getTSRole().getRoleName() + ",");
						}
						roleName = stringBuffer.toString().substring(0, stringBuffer.toString().length() - 1);
						cfe.setUserKey(roleName);
					}
				}
				cfeList.add(cfe);
			}
		}

		TagUtil.datagrid(response, dataGrid);
	}

	/**
	 * 用户删除选择对话框
	 * 
	 * @return
	 */
	@RequestMapping(params = "deleteDialog")
	public String deleteDialog(TSUser user,HttpServletRequest request) {
		request.setAttribute("user", user);
		return "system/user/user-delete";
	} 

	@RequestMapping(params = "delete")
	@ResponseBody
	public AjaxJson delete(TSUser user, @RequestParam String deleteType, HttpServletRequest req) {
		if (deleteType.equals("delete")) {
			return this.del(req);
		}else if (deleteType.equals("deleteTrue")) {
			return this.trueDel(user, req);
		}else{
			AjaxJson j = new AjaxJson();

			j.setMsg("删除逻辑参数异常,请重试.");
			return j;
		}
	}


	/**
	 * 用户信息录入和更新
	 * 
	 * @param req
	 * @return
	 */
	@RequestMapping(params = "del")
	@ResponseBody
	public AjaxJson del(HttpServletRequest req) {
		String message = null;
		String id = req.getParameter("id");
		AjaxJson j = new AjaxJson();
		TSUser user=null;
		try{
			user = userService.getById(id);
			user.setDeleteFlag(Globals.Delete_Forbidden);
			user.setStatus(Globals.User_Delete);//已删除
			userService.updateEntitie(user);
			systemService.addLog("删除用户：" + user.getUserName() + "成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
			logger.debug("删除用户：" + user.getUserName() + "成功");
			message = "用户：" + user.getUserName() + "删除成功";
			j.setMsg(message);
		}catch(Exception e){
			logger.error("执行失败：" + e.getMessage());
			systemService.addLog("删除用户：" + user.getUserName() + "失败，" + e.getMessage(), Globals.MODULE_SYSTEM, Globals.Log_Leavel_ERROR,"0");
			logger.error("删除用户：" + user.getUserName() + "失败," + e.getMessage());
		}
		return j;
	}

	/**
	 * 真实删除
	 * @param user
	 * @param req
	 * @return
	 */
	@RequestMapping(params = "trueDel")
	@ResponseBody
	public AjaxJson trueDel(TSUser user, HttpServletRequest req) {
		String message = null;
		AjaxJson j = new AjaxJson();
		if("admin".equals(user.getUserName())){
			message = "超级管理员[admin]不可删除";
			j.setMsg(message);
			return j;
		}
		user = systemService.getEntity(TSUser.class, user.getId());

		/*List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		if (!user.getStatus().equals(Globals.User_ADMIN)) {
			if (roleUser.size()>0) {
				// 删除用户时先删除用户和角色关系表
				delRoleUser(user);
                systemService.executeSql("delete from t_s_user_org where user_id=?", user.getId()); // 删除 用户-机构 数据
                userService.delete(user);
				message = "用户：" + user.getUserName() + "删除成功";
				systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
			} else {
				userService.delete(user);
				message = "用户：" + user.getUserName() + "删除成功";
			}
		} else {
			message = "超级管理员不可删除";
		}*/

		try {
			message = userService.trueDel(user);
		} catch (Exception e) {
			logger.error("执行失败：" + e.getMessage());
			message ="删除失败";
		}


		j.setMsg(message);
		return j;
	}

	/*public void delRoleUser(TSUser user) {
		// 同步删除用户角色关联表
		List<TSRoleUser> roleUserList = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		if (roleUserList.size() >= 1) {
			for (TSRoleUser tRoleUser : roleUserList) {
				systemService.delete(tRoleUser);
			}
		}
	}*/
	/**
	 * 检查用户名
	 * 
	 * @return
	 */
	@RequestMapping(params = "checkUser")
	@ResponseBody
	public ValidForm checkUser(HttpServletRequest request) {
		ValidForm v = new ValidForm();
		String userName=oConvertUtils.getString(request.getParameter("param"));
		String code=oConvertUtils.getString(request.getParameter("code"));
		List<TSUser> roles=systemService.findByProperty(TSUser.class,"userName",userName);
		if(roles.size()>0&&!code.equals(userName))
		{
			v.setInfo("用户名已存在");
			v.setStatus("n");
		}
		return v;
	}

	/**
	 * 用户录入
	 * 
	 * @param user
	 * @param req
	 * @return
	 */
	@RequestMapping(params = "saveUser")
	@ResponseBody
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		String message = null;
		AjaxJson j = new AjaxJson();
		HttpSession session = req.getSession();
		String randomStr=oConvertUtils.getString(req.getParameter("randomStr"));
		Client client = ClientManager.getInstance().getClient();
		String str = client.getRandomStr();
		//验证会话重放攻击
		if(!randomStr.equals(str)){
			session.removeAttribute(randomStr);
			j.setMsg("此次操作存在安全隐患");
			j.setSuccess(false);
			systemService.addLoginFaildLog("用户"+user.getUserName()+"此次操作存在问题！",
					Globals.Log_Type_LOGIN, Globals.Log_Leavel_INFO, "0",user.getUserName());
			return j;
		}else {
			client.setRandomStr(RandomStr.createRandomString(15));
		}
		if (!StringUtil.isNotEmpty(user.getId())) {
			//用户名不能是admin,root,guest,anonymous,administrator
			String checkUser = user.getUserName().toLowerCase(Locale.ENGLISH);
			if(("admin").equals(checkUser) || ("root").equals(checkUser)
				|| ("guest").equals(checkUser) ||("anonymous").equals(checkUser)
				|| ("administrator").equals(checkUser)) {
				j.setMsg("用户名不能是admin,root,guest,anonymous,administrator！");
				j.setSuccess(false);
				return j;
			}
			//获取密钥
			String password=oConvertUtils.getString(req.getParameter("newpassword"));
			String newpass="";//待解密新密码
			String sm3newpass="";//待解密新密码
			try{
				newpass = SM2Utils.decrypt(SM2Key.DBPrivKey, password);
				String[] smnewpass = SM2Utils.sm3decrypt(newpass);
				sm3newpass=smnewpass[0];
			}catch(Exception e){
				logger.error("error:",e);
			}
			//将用户的密码解密
			//判断密码强度
			//判断密码是否与用户名相同
			if(sm3newpass.equals(user.getUserName())){
				j.setMsg("密码不能与用户名相同，请重新输入！");
				j.setSuccess(false);
				return j;
			}
			if(sm3newpass.contains(user.getUserName())){
				j.setMsg("密码不能包含用户名，请重新输入！");
				j.setSuccess(false);
				return j;
			}
			if (sm3newpass.length()<8 && newpass.length()>20) {
				j.setMsg("密码长度为8-20位！");
				j.setSuccess(false);
				return j;
			}
			if (!CheckPassword.checkContent(sm3newpass)) {
				j.setMsg("密码必须包含字母数字和特定范围的特殊字符！");
				j.setSuccess(false);
				return j;
			}
			user.setPassword(password);
			
		}
		// 得到用户的角色
		String roleid = oConvertUtils.getString(req.getParameter("roleid"));
		//校验
		if(!(Check.checklen(user.getOfficePhone(), 0, 20, false)&&
				Check.checklen(user.getMobilePhone(), 0, 11, false)&&
				Check.checklen(user.getUserIp(), 0, 200, true)&&
				Check.checklen(user.getRealName(), 0, 50, false)&&
				Check.checklen(user.getEmail(), 0, 50, true)&&
				Check.checklen(user.getUserName(), 1, 10, true)
				))
		{
			j.setMsg("数据长度超过限制,请按要求输入！");
			j.setSuccess(false);
			return j;
		}
		if(!Check.checkUserName(user.getUserName()))
		{
			j.setMsg("用户名格式错误,请按要求输入！");
			j.setSuccess(false);
			return j;
		}
		if(!Check.checkip(user.getUserIp()))
		{
			j.setMsg("ip地址格式错误,请按要求输入！");
			j.setSuccess(false);
			return j;
		}
		if(!Check.checktele(user.getMobilePhone()))
		{
			j.setMsg("手机号码格式错误,请按要求输入！");
			j.setSuccess(false);
			return j;
		}
		if(!Check.checkemail(user.getEmail()))
		{
			j.setMsg(user.getEmail()+"邮箱格式错误,请按要求输入！");
			j.setSuccess(false);
			return j;
		}
		if (StringUtil.isNotEmpty(user.getId())) {
			String orgIds = oConvertUtils.getString(req.getParameter("orgIds"));
			TSUser users = systemService.getEntity(TSUser.class, user.getId());
			users.setEmail(user.getEmail().replace("&#64;", "@"));
			users.setDepartid(orgIds);
			users.setOfficePhone(user.getOfficePhone());
			users.setMobilePhone(user.getMobilePhone());
			users.setUserIp(user.getUserIp());
			users.setType(user.getType());
			systemService.executeSql("delete from t_s_user_org where user_id=?", user.getId());
			saveUserOrgList(req, user);
			//            users.setTSDepart(user.getTSDepart());
			//判断用户的状态流转
			//只有处于新建、激活、编辑状态的用户可以编辑，新建、编辑经过编辑之后还是处于当前状态，激活的用户编辑之后变为编辑状态，审核状态都变为待审核

			users.setUserKey(user.getUserKey());
			String begin[]=user.getBeginDate().split(":");
			String end[]=user.getEndDate().split(":");
			//修改用户状态为修改
			users.setStatus(Globals.User_Edit);
			//修改用户，用户审核状态设置为待审核状态
			users.setCheckStatus(Globals.User_Audit_Wait);
			users.setBeginDate(begin[0]);
			users.setEndDate(end[0]);
			users.setRealName(user.getRealName());
			users.setStatus(Globals.User_Edit);
			users.setActivitiSync(user.getActivitiSync());			
			systemService.updateEntitie(users);
			List<TSRoleUser> ru = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
			systemService.deleteAllEntitie(ru);
			message = "用户: " + users.getUserName() + "更新成功，等待审核";
			if (StringUtil.isNotEmpty(roleid)) {
				saveRoleUser(users, roleid);
			}
			systemService.addLog("更新用户"+users.getUserName()+"成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
		} else {
			TSUser users = systemService.findUniqueByProperty(TSUser.class, "userName",user.getUserName());
			if (users != null) {
				message = "用户: " + users.getUserName() + "已经存在";
			}else {
				String orgIds = oConvertUtils.getString(req.getParameter("orgIds"));
				String begin[]=user.getBeginDate().split(":");
				String end[]=user.getEndDate().split(":");
				user.setPass_update_flag((short)0);
				user.setDepartid(orgIds);
				user.setBeginDate(begin[0]);
				user.setEndDate(end[0]);
				user.setStatus(Globals.User_New);
				user.setDeleteFlag(Globals.Delete_Normal);
				//新建用户 用户审核状态设置为待审核状态
				user.setCheckStatus(Globals.User_Audit_Wait);
				user.setEmail(user.getEmail().replace("&#64;", "@"));
				systemService.save(user);
				// todo zhanggm 保存多个组织机构
				saveUserOrgList(req, user);
				message = "用户: " + user.getUserName() + "添加成功，等待审核";
				if (StringUtil.isNotEmpty(roleid)) {
					saveRoleUser(user, roleid);
				}
				systemService.addLog("添加用户"+user.getUserName()+"成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
				//修改密码也在密码校验表中更新用户的密码
				SysPassValidate passValidate = new SysPassValidate();
				passValidate.setPassword(user.getPassword());
				passValidate.setUpdateTime(new Date());
				passValidate.setUserId(user.getId());
				passValidate.setUserName(user.getUserName());
				systemService.save(passValidate);
			}

		}
		j.setMsg(message);

		return j;
	}

	/**
	 * 保存 用户-组织机构 关系信息
	 * @param request request
	 * @param user user
	 */
	private void saveUserOrgList(HttpServletRequest request, TSUser user) {
		String orgIds = oConvertUtils.getString(request.getParameter("orgIds"));

		List<TSUserOrg> userOrgList = new ArrayList<TSUserOrg>();
		List<String> orgIdList = extractIdListByComma(orgIds);
		for (String orgId : orgIdList) {
			TSDepart depart = new TSDepart();
			depart.setId(orgId);

			TSUserOrg userOrg = new TSUserOrg();
			userOrg.setTsUser(user);
			userOrg.setTsDepart(depart);

			userOrgList.add(userOrg);
		}
		if (!userOrgList.isEmpty()) {
			systemService.batchSave(userOrgList);
		}
	}


	protected void saveRoleUser(TSUser user, String roleidstr) {
		String[] roleids = roleidstr.split(",");
		for (int i = 0; i < roleids.length; i++) {
			TSRoleUser rUser = new TSRoleUser();
			TSRole role = systemService.getEntity(TSRole.class, roleids[i]);
			rUser.setTSRole(role);
			rUser.setTSUser(user);
			systemService.save(rUser);

		}
	}

	/**
	 * 用户选择角色跳转页面
	 * 
	 * @return
	 */
	@RequestMapping(params = "roles")
	public ModelAndView roles(HttpServletRequest request) {
		//--author：zhoujf-----start----date:20150531--------for: 编辑用户，选择角色,弹出的角色列表页面，默认没选中
		ModelAndView mv = new ModelAndView("system/user/users");
		String ids = oConvertUtils.getString(request.getParameter("ids"));
		mv.addObject("ids", ids);
		return mv;
	}

	/**
	 * 角色显示列表
	 * 
	 * @param request
	 * @param response
	 * @param dataGrid
	 */
	@RequestMapping(params = "datagridRole")
	public void datagridRole(TSRole tsRole, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSRole.class, dataGrid);
		//查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsRole);
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}

	/**
	 * easyuiAJAX请求数据： 用户选择角色列表
	 * 
	 * @param user
	 */
	@RequestMapping(params = "addorupdate")
	public ModelAndView addorupdate(TSUser user, HttpServletRequest req) {

		/*		List<TSDepart> departList = new ArrayList<TSDepart>();
		String departid = oConvertUtils.getString(req.getParameter("departid"));
		if(!StringUtil.isEmpty(departid)){
			departList.add((TSDepart)systemService.getEntity(TSDepart.class,departid));
		}else {
			departList.addAll((List)systemService.getList(TSDepart.class));
		}
		req.setAttribute("departList", departList);*/

		List<String> orgIdList = new ArrayList<String>();
		TSDepart tsDepart = new TSDepart();
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
			req.setAttribute("user", user);
			idandname(req, user);
			getOrgInfos(req, user);
		}

		req.setAttribute("tsDepart", tsDepart);
		//req.setAttribute("orgIdList", JSON.toJSON(orgIdList));


		//        return new ModelAndView("system/user/user");
		return new ModelAndView("iesp/system/addTSUser");
	}

	/**
	 * 用户的登录后的组织机构选择页面
	 * @param request request
	 * @return 用户选择组织机构页面
	 */
	@RequestMapping(params = "userOrgSelect")
	public ModelAndView userOrgSelect(HttpServletRequest request) {
		List<TSDepart> orgList = new ArrayList<TSDepart>();
		String userId = oConvertUtils.getString(request.getParameter("userId"));

		List<Object[]> orgArrList = systemService.findHql("from TSDepart d,TSUserOrg uo where d.id=uo.tsDepart.id and uo.tsUser.id=?", new String[]{userId});
		for (Object[] departs : orgArrList) {
			orgList.add((TSDepart) departs[0]);
		}
		request.setAttribute("orgList", orgList);

		TSUser user = systemService.getEntity(TSUser.class, userId);
		request.setAttribute("user", user);

		return new ModelAndView("system/user/userOrgSelect");
	}


	public void idandname(HttpServletRequest req, TSUser user) {
		List<TSRoleUser> roleUsers = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
        StringBuffer stringBuffer1 = new StringBuffer();
        StringBuffer stringBuffer2 = new StringBuffer();
		if (roleUsers.size() > 0) {
			for (TSRoleUser tRoleUser : roleUsers) {
                stringBuffer1.append(tRoleUser.getTSRole().getId() + ",");
                stringBuffer2.append(tRoleUser.getTSRole().getRoleName() + ",");
			}
		}
		req.setAttribute("id", stringBuffer1.toString());
		req.setAttribute("roleName", stringBuffer2.toString());

	}

	public void getOrgInfos(HttpServletRequest req, TSUser user) {
		List<TSUserOrg> tSUserOrgs = systemService.findByProperty(TSUserOrg.class, "tsUser.id", user.getId());
        StringBuffer stringBufferOrgIds = new StringBuffer();
        StringBuffer stringBufferDepartname = new StringBuffer();
		if (tSUserOrgs.size() > 0) {
			for (TSUserOrg tSUserOrg : tSUserOrgs) {
                stringBufferOrgIds.append( tSUserOrg.getTsDepart().getId() + ",");
                stringBufferDepartname.append(tSUserOrg.getTsDepart().getDepartname() + ",");
			}
		}
		req.setAttribute("orgIds", stringBufferOrgIds.toString());
		req.setAttribute("departname", stringBufferDepartname.toString());

	}

	/**
	 * 根据部门和角色选择用户跳转页面
	 */
	@RequestMapping(params = "choose")
	public String choose(HttpServletRequest request) {
		List<TSRole> roles = systemService.loadAll(TSRole.class);
		request.setAttribute("roleList", roles);
		return "system/membership/checkuser";
	}

	/**
	 * 部门和角色选择用户的panel跳转页面
	 * 
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "chooseUser")
	public String chooseUser(HttpServletRequest request) {
		String departid = request.getParameter("departid");
		String roleid = request.getParameter("roleid");
		request.setAttribute("roleid", roleid);
		request.setAttribute("departid", departid);
		return "system/membership/userlist";
	}

	/**
	 * 部门和角色选择用户的用户显示列表
	 * 
	 * @param request
	 * @param response
	 * @param dataGrid
	 */
	@RequestMapping(params = "datagridUser")
	public void datagridUser(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		String departid = request.getParameter("departid");
		String roleid = request.getParameter("roleid");
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);

			if(departid!=null && StringUtils.isNotEmpty(departid)){
				if (departid.length() > 0) {
					cq.eq("TDepart.departid", oConvertUtils.getInt(departid, 0));
					cq.add();
				}
			}
            StringBuilder stringBuilder = new StringBuilder();
			if(roleid!=null && StringUtils.isNotEmpty(roleid)){
				if (roleid.length() > 0) {
					List<TSRoleUser> roleUsers = systemService.findByProperty(TSRoleUser.class, "TRole.roleid", oConvertUtils.getInt(roleid, 0));
					if (roleUsers.size() > 0) {
						for (TSRoleUser tRoleUser : roleUsers) {
							stringBuilder.append(tRoleUser.getTSUser().getId() + ",");
						}
					}
					cq.in("userid", oConvertUtils.getInts(stringBuilder.toString().split(",")));
					cq.add();
				}
			}
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}

	/**
	 * 根据部门和角色选择用户跳转页面
	 */
	@RequestMapping(params = "roleDepart")
	public String roleDepart(HttpServletRequest request) {
		List<TSRole> roles = systemService.loadAll(TSRole.class);
		request.setAttribute("roleList", roles);
		return "system/membership/roledepart";
	}

	/**
	 * 部门和角色选择用户的panel跳转页面
	 * 
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "chooseDepart")
	public ModelAndView chooseDepart(HttpServletRequest request) {
		String nodeid = request.getParameter("nodeid");
		ModelAndView modelAndView = null;
		if(nodeid!=null &&!StringUtils.isNotEmpty(nodeid)){
			if (nodeid.equals("role")) {
				modelAndView = new ModelAndView("system/membership/users");
			} else {
				modelAndView = new ModelAndView("system/membership/departList");
			}
		}
		return modelAndView;
	}

	/**
	 * 部门和角色选择用户的用户显示列表
	 * 
	 * @param request
	 * @param response
	 * @param dataGrid
	 */
	@RequestMapping(params = "datagridDepart")
	public void datagridDepart(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSDepart.class, dataGrid);
		systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}

	/**
	 * 测试
	 * 
	 * @return
	 */
	@RequestMapping(params = "test")
	public void test(HttpServletRequest request, HttpServletResponse response) {
		String jString = request.getParameter("_dt_json");
		DataTables dataTables = new DataTables(request);
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataTables);
		String username = request.getParameter("userName");
		if (username != null) {
			cq.like("userName", username);
			cq.add();
		}
		DataTableReturn dataTableReturn = systemService.getDataTableReturn(cq, true);
		TagUtil.datatable(response, dataTableReturn, "id,userName,mobilePhone,TSDepart_departname");
	}

	/**
	 * 用户列表页面跳转
	 * 
	 * @return
	 */
	@RequestMapping(params = "index")
	public String index() {
		return "bootstrap/main";
	}

	/**
	 * 用户列表页面跳转
	 * 
	 * @return
	 */
	@RequestMapping(params = "main")
	public String main() {
		return "bootstrap/test";
	}

	/**
	 * 测试
	 * 
	 * @return
	 */
	@RequestMapping(params = "testpage")
	public String testpage(HttpServletRequest request) {
		return "test/test";
	}

	/**
	 * 设置签名跳转页面
	 * 
	 * @param request
	 * @return
	 */
	@RequestMapping(params = "addsign")
	public ModelAndView addsign(HttpServletRequest request) {
		String id = request.getParameter("id");
		request.setAttribute("id", id);
		return new ModelAndView("system/user/usersign");
	}

	/**
	 * 用户录入
	 * 
	 * @param req
	 * @return
	 */

	@RequestMapping(params = "savesign", method = RequestMethod.POST)
	@ResponseBody
	public AjaxJson savesign(HttpServletRequest req) {
		String message = null;
		UploadFile uploadFile = new UploadFile(req);
		String id = uploadFile.get("id");
		TSUser user = systemService.getEntity(TSUser.class, id);
		uploadFile.setRealPath("signatureFile");
		uploadFile.setCusPath("signature");
		uploadFile.setByteField("signature");
		uploadFile.setBasePath("resources");
		uploadFile.setRename(false);
		uploadFile.setObject(user);
		AjaxJson j = new AjaxJson();
		message = user.getUserName() + "设置签名成功";
		systemService.uploadFile(uploadFile);
		systemService.addLog(message, Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
		j.setMsg(message);

		return j;
	}
	/**
	 * 测试组合查询功能
	 * @param user
	 * @param request
	 * @param response
	 * @param dataGrid
	 */
	@RequestMapping(params = "testSearch")
	public void testSearch(TSUser user, HttpServletRequest request,HttpServletResponse response,DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		if(user.getUserName()!=null){
            String s = StringTransUtil.stringReplace(user.getUserName());
            cq.like("userName", s);
		}
		if(user.getRealName()!=null){
            String s = StringTransUtil.stringReplace(user.getRealName());
            cq.like("realName", s);
		}
		cq.add();
		this.systemService.getDataGridReturn(cq, true);
		TagUtil.datagrid(response, dataGrid);
	}
	@RequestMapping(params = "changestyle")
	public String changeStyle(HttpServletRequest request) {
		TSUser user = ResourceUtil.getSessionUserName();
		if(user==null){
			return "login/login";
		}
		//		String indexStyle = "shortcut";
		//		String cssTheme="";
		//		Cookie[] cookies = request.getCookies();
		//		for (Cookie cookie : cookies) {
		//			if(cookie==null || StringUtils.isEmpty(cookie.getName())){
		//				continue;
		//			}
		//			if(cookie.getName().equalsIgnoreCase("JEECGINDEXSTYLE")){
		//				indexStyle = cookie.getValue();
		//			}
		//			if(cookie.getName().equalsIgnoreCase("JEECGCSSTHEME")){
		//				cssTheme = cookie.getValue();
		//			}
		//		}
		SysThemesEnum sysThemesEnum = SysThemesUtil.getSysTheme(request);
		request.setAttribute("indexStyle", sysThemesEnum.getStyle());
		//		request.setAttribute("indexStyle", "hplus");
		//		request.setAttribute("cssTheme", cssTheme);
		return "system/user/changestyle";
	}
	/**
	 * @Title: saveStyle
	 * @Description: 修改首页样式
	 * @param request
	 * @return AjaxJson    
	 * @throws
	 */
	@RequestMapping(params = "savestyle")
	@ResponseBody
	public AjaxJson saveStyle(HttpServletRequest request,HttpServletResponse response) {
		AjaxJson j = new AjaxJson();
		j.setSuccess(Boolean.FALSE);
		TSUser user = ResourceUtil.getSessionUserName();
		if(user!=null){
		   /* String indexStyle = request.getParameter("indexStyle");
			if (indexStyle != null && StringUtils.isNotEmpty(indexStyle)) {
				logger.info(String.format("indexStyle: %s",indexStyle));
			}*/
		    //			String cssTheme = request.getParameter("cssTheme");

			//			if(StringUtils.isNotEmpty(cssTheme)){
			//				Cookie cookie4css = new Cookie("JEECGCSSTHEME", cssTheme);
			//				cookie4css.setMaxAge(3600*24*30);
			//				response.addCookie(cookie4css);
			//				logger.info("cssTheme:"+cssTheme);
			//			}else if("ace".equals(indexStyle)){
			//				Cookie cookie4css = new Cookie("JEECGCSSTHEME", "metro");
			//				cookie4css.setMaxAge(3600*24*30);
			//				response.addCookie(cookie4css);
			//				logger.info("cssTheme:metro");

			//			}else {
			//				Cookie cookie4css = new Cookie("JEECGCSSTHEME", "");
			//				cookie4css.setMaxAge(3600*24*30);
			//				response.addCookie(cookie4css);
			//				logger.info("cssTheme:default");
			//			}

			/*if(Pattern.matches("[0-9A-Za-z]+",indexStyle)){
				if(StringUtils.isNotEmpty(indexStyle)){
					Cookie cookie = new Cookie("JEECGINDEXSTYLE", indexStyle);
					//设置cookie有效期为一个月
					cookie.setMaxAge(3600*24*30);
					cookie.setSecure(true);
					response.addCookie(cookie);
                    if(Pattern.matches("[A-Za-z0-9_]+-",indexStyle)) {
						logger.debug(String.format(" ----- 首页样式: indexStyle ----- %s",indexStyle));
                    }else{
                        logger.debug(" ----- 首页样式: indexStyle ----- 有误");
                    }
					j.setSuccess(Boolean.TRUE);
					j.setMsg("样式修改成功，请刷新页面");
				}
			}
*/
			try {
				ClientManager.getInstance().getClient().getFunctions().clear();
			} catch (Exception e) {
				logger.error("error:",e);
			}

		}else{
			j.setMsg("请登录后再操作");
		}
		return j;
	}

	/**
	 * 导入功能跳转
	 *
	 * @return
	 */
	@RequestMapping(params = "upload")
	public ModelAndView upload(HttpServletRequest req) {
		req.setAttribute("controller_name","userController");
		return new ModelAndView("common/upload/pub_excel_upload");
	}

	/**
	 * 导出excel
	 *
	 * @param request
	 * @param response
	 */
	@RequestMapping(params = "exportXls")
	public String exportXls(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
			, DataGrid dataGrid,ModelMap modelMap) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, tsUser, request.getParameterMap());
		List<TSUser> tsUsers = this.userService.getListByCriteriaQuery(cq,false);
		//导出的时候处理一下组织机构编码和角色编码
		for(int i=0;i<tsUsers.size();i++){
			TSUser user = tsUsers.get(i);
			//托管
			systemService.getSession().evict(user);
			String id = user.getId();
			List<TSRole> roles = systemService.getSession().createSQLQuery("select * from t_s_role where id in (select roleid from t_s_role_user where userid=:userid)")
					.addEntity(TSRole.class).setString("userid",id).list();
            StringBuffer stringBuffer = new StringBuffer();
			for(TSRole role:roles){
                stringBuffer.append(role.getRoleCode()+",");
			}
			user.setUserKey(stringBuffer.toString().substring(0,stringBuffer.toString().length()-1));
			List<TSDepart> departs = systemService.getSession().createSQLQuery("select * from t_s_depart where id in (select org_id from t_s_user_org where user_id=:userid)")
					.addEntity(TSDepart.class).setString("userid",id).list();
            StringBuilder stringBuilder = new StringBuilder();
			for(TSDepart depart:departs){
                stringBuilder.append(depart.getOrgCode()+",");
			}
			user.setDepartid(stringBuilder.toString().substring(0,stringBuilder.toString().length()-1));
		}
		modelMap.put(NormalExcelConstants.FILE_NAME,"用户表");
		modelMap.put(NormalExcelConstants.CLASS,TSUser.class);
		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("用户表列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
				"导出信息"));
		modelMap.put(NormalExcelConstants.DATA_LIST,tsUsers);
		return NormalExcelConstants.JEECG_EXCEL_VIEW;
	}

	/**
	 * 导出excel 使模板
	 *
	 * @param request
	 * @param response
	 */
	@RequestMapping(params = "exportXlsByT")
	public String exportXlsByT(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
			, DataGrid dataGrid,ModelMap modelMap) {
		modelMap.put(NormalExcelConstants.FILE_NAME,"用户表");
		modelMap.put(NormalExcelConstants.CLASS,TSUser.class);
		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("用户表列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
				"导出信息"));
		modelMap.put(NormalExcelConstants.DATA_LIST,new ArrayList());
		return NormalExcelConstants.JEECG_EXCEL_VIEW;
	}

	@SuppressWarnings("unchecked")
	@RequestMapping(params = "importExcel", method = RequestMethod.POST)
	@ResponseBody
	public AjaxJson importExcel(HttpServletRequest request, HttpServletResponse response) {
		AjaxJson j = new AjaxJson();

		MultipartHttpServletRequest multipartRequest = (MultipartHttpServletRequest) request;
		Map<String, MultipartFile> fileMap = multipartRequest.getFileMap();
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			MultipartFile file = entity.getValue();// 获取上传文件对象
			ImportParams params = new ImportParams();
			params.setTitleRows(2);
			params.setHeadRows(1);
			params.setNeedSave(true);
			try {
				List<TSUser> tsUsers = ExcelImportUtil.importExcel(file.getInputStream(),TSUser.class,params);
				for (TSUser tsUser : tsUsers) {
					tsUser.setStatus(new Short("1"));
					String username = tsUser.getUserName();
					String roleCodes = tsUser.getUserKey();
					String deptCodes = tsUser.getDepartid();

					if(username==null || StringUtils.isEmpty(username)){
						j.setMsg("用户名为必填字段，导入失败");
					}else if((roleCodes == null && StringUtils.isEmpty(roleCodes))||(deptCodes==null||StringUtils.isEmpty(deptCodes))){
						List<TSUser> users = systemService.findByProperty(TSUser.class,"userName",username);
						if(users.size()!=0){
							//用户存在更新
							TSUser user = users.get(0);
							MyBeanUtils.copyBeanNotNull2Bean(tsUser,user);
							user.setDepartid(null);
							systemService.saveOrUpdate(user);
						}else{
							tsUser.setDepartid(null);
							systemService.save(tsUser);
						}
					}else{
						String[] roles = roleCodes.split(",");
						String[] depts = deptCodes.split(",");
						boolean flag = true;
						//判断组织机构编码和角色编码是否存在，如果不存在，也不能导入
						for(String roleCode:roles){
							List<TSRole> roleList = systemService.findByProperty(TSRole.class,"roleCode",roleCode);
							if(roleList.size()==0){
								flag = false;
							}
						}

						for(String deptCode:depts){
							List<TSDepart> departList = systemService.findByProperty(TSDepart.class,"orgCode",deptCode);
							if(departList.size()==0){
								flag = false;
							}
						}

						if(flag){
							//判断用户是否存在
							List<TSUser> users = systemService.findByProperty(TSUser.class,"userName",username);
							if(users.size()!=0){
								//用户存在更新
								TSUser user = users.get(0);
								MyBeanUtils.copyBeanNotNull2Bean(tsUser,user);
								user.setDepartid(null);
								systemService.saveOrUpdate(user);

								String id = user.getId();
								systemService.executeSql("delete from t_s_role_user where userid='"+id+"'");
								for(String roleCode:roles){
									//根据角色编码得到roleid
									List<TSRole> roleList = systemService.findByProperty(TSRole.class,"roleCode",roleCode);
									TSRoleUser tsRoleUser = new TSRoleUser();
									tsRoleUser.setTSUser(user);
									tsRoleUser.setTSRole(roleList.get(0));
									systemService.save(tsRoleUser);
								}

								systemService.executeSql("delete from t_s_user_org where user_id='"+id+"'");
								for(String orgCode:depts){
									//根据角色编码得到roleid
									List<TSDepart> departList = systemService.findByProperty(TSDepart.class,"orgCode",orgCode);
									TSUserOrg tsUserOrg = new TSUserOrg();
									tsUserOrg.setTsDepart(departList.get(0));
									tsUserOrg.setTsUser(user);
									systemService.save(tsUserOrg);
								}
							}else{
								//不存在则保存
								//TSUser user = users.get(0);
								tsUser.setDepartid(null);
								systemService.save(tsUser);
								for(String roleCode:roles){
									//根据角色编码得到roleid
									List<TSRole> roleList = systemService.findByProperty(TSRole.class,"roleCode",roleCode);
									TSRoleUser tsRoleUser = new TSRoleUser();
									tsRoleUser.setTSUser(tsUser);
									tsRoleUser.setTSRole(roleList.get(0));
									systemService.save(tsRoleUser);
								}

								for(String orgCode:depts){
									//根据角色编码得到roleid
									List<TSDepart> departList = systemService.findByProperty(TSDepart.class,"orgCode",orgCode);
									TSUserOrg tsUserOrg = new TSUserOrg();
									tsUserOrg.setTsDepart(departList.get(0));
									tsUserOrg.setTsUser(tsUser);
									systemService.save(tsUserOrg);
								}
							}
							j.setMsg("文件导入成功！");
						}else {
							j.setMsg("组织机构编码和角色编码不能匹配");
						}
					}
				}
			} catch (Exception e) {
				j.setMsg("文件导入失败！");
				logger.error(ExceptionUtil.getExceptionMessage(e));
			}finally{
				try {
					file.getInputStream().close();
				} catch (IOException e) {
					logger.error("执行失败：" + e.getMessage());
				}
			}
		}
		return j;
	}

	/**
	 * 选择用户跳转页面
	 *
	 * @return
	 */
	@RequestMapping(params = "userSelect")
	public String userSelect() {
		return "system/user/userSelect";
	}

	//----------------------------------异常告警start------------------------------------------------
	@RequestMapping(params="getExceptionList")
	@ResponseBody
	public List<SysException> getExceptionList(Boolean isAdmin,Boolean isAudit,HttpServletRequest request){
		List<SysException> exceptions = new ArrayList<>();
		try{
			List<SysException> all = systemService.findByProperty(SysException.class, "isRead", (short)0);
			if(isAdmin) {
				for(SysException exception:all) {
					if(!exception.getExceptionType().equals(Globals.Exception_type_Log_Max)) {
						exceptions.add(exception);
					}
				}
			}else if(isAudit) {
				for(SysException exception:all) {
					if(exception.getExceptionType().equals(Globals.Exception_type_Log_Max)) {
						exceptions.add(exception);
					}
				}
			}
		}catch(Exception e){
			logger.error("执行失败：" + e.getMessage());
		}
		return exceptions;
	}

	/**
	 * 阅读通知公告
	 * @param req
	 * @return
	 */
	@RequestMapping(params = "updateExceptionRead")
	@ResponseBody
	public AjaxJson updateExceptionRead(String exceptionId,HttpServletRequest req) {
		AjaxJson j = new AjaxJson();
		j.setSuccess(true);
		try {
			SysException exception = systemService.getEntity(SysException.class, exceptionId);
			exception.setIsRead((short)1);
			systemService.saveOrUpdate(exception);
			systemService.addLog("确认严重异常[" + exception.getTitle() + "]", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO, "1");
		} catch (Exception e) {
			logger.error("执行失败：" + e.getMessage());
			systemService.addLog("确认严重异常失败：" + e.getMessage(), Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO, "0");
			j.setSuccess(false);
		}
		return j;
	}

	//----------------------------------异常告警end--------------------------------------------

	/***************************日志配置开始***************************/
	/**
	 * 日志配置
	 * @return
	 */
	@RequestMapping(params = "logConfigList")
	public ModelAndView logConfigList() {
		return new ModelAndView("iesp/log/logConfigList");
	}

	/*@RequestMapping(params = "getLogConfigList", method = RequestMethod.GET)
	@ResponseBody
	public JSONObject getLogConfigList(Integer limit, Integer offset, HttpServletRequest request) {
		JSONObject result = new JSONObject();
		JSONArray array = new JSONArray();
		try{
			Pagination<LogConfig> pagination = userService.getLogConfigList(limit, offset, null);
			for (LogConfig logConfig : pagination.getItems()) {
				JSONObject obj = new JSONObject();
				obj.put("id", logConfig.getId());
				obj.put("configName", logConfig.getConfigName());
				obj.put("code", logConfig.getCode());
				obj.put("flag", logConfig.getFlag());
				array.add(obj);
			}
			systemService.addLog("查看日志配置列表成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
			logger.debug("查看日志配置列表成功");
			result.put("total", pagination.getRowsCount());
			result.put("rows", array);
			return result;
		}catch(Exception e){
			systemService.addLog("查看日志配置列表成功", Globals.MODULE_SYSTEM,Globals.Log_Leavel_ERROR, "0");
			logger.debug("查看日志配置列表成功");
			return null;
		}
	}*/

	@RequestMapping(params="updateLogConfigFlag")
	@ResponseBody
	public JSONObject updateLogConfigFlag(Integer id,Integer flag,HttpServletRequest request) {
		JSONObject result = new JSONObject();
		try {
			Integer res = userService.updateFlag(id, flag);
			if(res==1) {
//				LogConfig logConfig = systemService.get(LogConfig.class, id);
				result.put("result", true);
				//更新缓存中信息
//				ResourceUtil.logConfigFlagMap.put(logConfig.getCode(), logConfig.getFlag());
				systemService.addLog("更新日志配置[" + id + "]的状态成功", Globals.MODULE_SYSTEM, Globals.Log_Leavel_INFO,"1");
				logger.debug("更新日志配置[\" + id + \"]的状态");
			}else {
				result.put("result", false);
				systemService.addLog("更新日志配置[\" + id + \"]的状态失败", Globals.MODULE_SYSTEM, Globals.Log_Leavel_ERROR,"0");
			}
		} catch (Exception e) {
			result.put("result", false);
			systemService.addLog("更新日志配置[\" + id + \"]的状态失败" + e.getMessage(), Globals.MODULE_SYSTEM, Globals.Log_Leavel_ERROR,"0");
			logger.error("更新日志配置[" + id + "]的状态失败" + e.getMessage());
		}
		return result;
	}

	/***************************日志配置结束***************************/
}