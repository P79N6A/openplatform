	...
	<div class="panel-body">
		<form class="form-horizontal" role="form" id="addForm" method="POST">
			...
			<div class="form-group">
				<label for="appName" class="col-sm-3 control-label">应用：</label>
				<div class="col-sm-9" style="padding-right:0px">
					<input id="appName" name="appName" type="text" class="form-control" readonly value="${connect.appName}"/>
				</div>
			</div>
			<div class="form-group">
				<label for="type" class="col-sm-3 control-label">申请类型：</label>
				<div class="col-sm-9" style="padding-right:0px">
					<select class="form-control" id="type" name="type" disabled style="cursor:pointer">
						<option <c:if test="${connect.type == 0}">selected</c:if> value="0">测试环境</option>
						<option <c:if test="${connect.type == 1}">selected</c:if> value="1">正式环境</option>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-9" style="padding-right:0px">
					<div class="input-group" style="width:100%">
						<input id="ip" name="ip" type="text" value="${connect.ip}" class="form-control input-sm"
							   placeholder="多个IP地址请用英文逗号,分割" />
					</div>
				</div>
			</div>
		</form>
...
<script type="text/javascript">
    $(document).ready(function() {
        $("#btn_sub").bind("click",function(){
            var options={
                url:"apiAppConnectController.do?doUpdate",
                type:"post",
                dataType:"json",
