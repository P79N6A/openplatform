		    ...
		    <form id="addForm" name="addForm" method="post" class="form-horizontal" role="form">
				...
				<c:if test="${type == 1}">
					<div class="form-group">
						...
						<div class="col-sm-8" style="padding-right:0px">
							<input type="number" id="num" name="num" min="0" style="width:79%" value="${chargeMode.num}" class="form-control col-sm-8" />
							<select name="unit" class="form-control col-sm-8"  style="width:20%">
								<option value="1" <c:if test="${chargeMode.unit == 1}">selected</c:if>>周</option>
								<option value="2" <c:if test="${chargeMode.unit == 2}">selected</c:if>>月</option>
								<option value="3" <c:if test="${chargeMode.unit == 3}">selected</c:if>>年</option>
							</select>
						</div>
					</div>
				</c:if>
					...
					<div class="form-group" id="num-div">
						...
						<div class="col-sm-8" style="padding-right:0px">
							<div class="input-group">
								<input type="number" id="startNum" min="0" name="startNum" value="${chargeMode.startNum}" class="form-control" />
								<label class="input-group-addon">~</label>
								<input type="number" id="endNum" min="0" name="endNum" value="${chargeMode.endNum}" class="form-control" />
				...
				<div class="form-group">
					...
					<div class="col-sm-8" style="padding-right:0px">
						<c:if test="${type == 2}">
							<div class="input-group">
								<input type="number" id="price" min="0" name="price" value="${chargeMode.price}" class="form-control" />
								<label class="input-group-addon" style="">元/次</label>
							</div>
						</c:if>
						<c:if test="${type != 2}">
							<input type="number" id="price" step="0.01" name="price" value="${chargeMode.price}" class="form-control" />
						</c:if>
					</div>
				</div>
		    </form>
