...
<body>
							...
							<form id="apiAppForm" onkeydown="if(event.keyCode==13){reloadTable();return false;}">
											<div class="col-xs-12 col-sm-6 col-md-4">
												...
												<div class="input-group" style="width: 100%">
													<input type="text" class="form-control input-sm" id="appName_search" name="appName"/>
												</div>
											</div>
	...
	<script type="text/javascript">
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#apiAppList').bootstrapTable({
						url : 'appController.do?datagrid', //请求后台的URL（*）
						method : 'post', //请求方式（*）
                            ...
                            },
                           {
									...
									formatter : function(value,row, index) {
                                        ...
                                        if (row.publishStatus != -1) {
                                            if(row.publishStatus == 1){
                                                href += "<button class='btn btn-xs btn-danger' style='margin-left:5px;float: left' title='作废' " +
                                                "onclick='deleteApiInfo(\"" + row.id + "\")'>" +
                                                "<span class='glyphicon glyphicon-trash'></span>作废</button>&nbsp";
											}else {
                                                href += "<button class='btn btn-xs btn-danger' style='margin-left:5px;float: left' title='作废' " +
                                                "onclick='deleteApiInfo(\"" + row.id + "\")'>" +
                                                "<span class='glyphicon glyphicon-trash'></span>作废</button>&nbsp";
                                            href += "<button class='btn btn-xs btn-primary' style='margin-left:12px' title='配置计费模型' " +
                                                "onclick='updateCharge(\"" + row.id + "\")'>" +
                                                "配置计费模型</button>&nbsp";
											}
                                        }
                                        if (row.publishStatus == 0) {
                                            href += "<button class='btn btn-xs btn-primary' style='margin-left:5px;float: right' title='发布' " +
                                                "onclick='publishApp(\"" + row.id + "\")'>" +
                                                "发布</button>";
                                        }
										return href;
									}
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,appName,appId,appDesc,appVersion,auditStatus,publishStatus'
				};
                ...
                callback: function (result) {
                    if (result) {
                        $.ajax({
                            type: "post",
                            dataType: "json",
                            url: "appController.do?publishApp",
                            data: {id: id},
                            success: function (data) {
                                if (data.success) {
                                    quickNotify(data.msg, "success");
        ...
        function  updateCharge(id) {
            bootOpenNormal('请选择计费模型','appController.do?goAppChargeModel&id=' + id,'wide')
        }
