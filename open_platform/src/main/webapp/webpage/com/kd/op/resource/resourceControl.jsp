    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#apiOrderList').bootstrapTable({
                url: 'resourceController.do?resourceControlDatagrid&listType=resource', //请求后台的URL（*）
                method: 'post', //请求方式（*）
                        ...
                        formatter: function (value, row, index) {
                            ...
                            if(row.auditStatus == '1' || row.auditStatus == '2'){
                                if(row.orderType == 'api'){
                                    href += "<button class='btn btn-xs btn-warning'  onclick='lookDetailNew(\"" + row.id + "\")'>" +
...
//            } else {
    ...
    //能力订单添加配置资源管理按钮
    function lookDetailNew(id) {
        var dialog = bootOpenLook('订单详情', 'order.do?lookDetail&type=order&id=' + id, 'wide');
    }
</script>
