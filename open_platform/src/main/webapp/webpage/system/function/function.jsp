...
<style>
.inputxt {height:30px;width:auto}
</style>
</head>
<body>
<t:formvalid formid="formobj" layout="div" dialog="true" refresh="true" action="functionController.do?saveFunction">
   ...
   <div class="form">
	...
	<div class="form" id="pfun">
        ...
        <input id="cc"  class="inputxt" <c:if test="${function.TSFunction.functionLevel eq 0}"> value="${function.TSFunction.id}"</c:if>
	...
	<div class="form">
      <label class="Validform_label">角色:</label>
               <input id="roleid" name="roleIds"   type="hidden" value="${roleid}"/>
               <input id="rolename" name="rolename" class="inputxt" placeholder="请选择角色" readonly="readonly" datatype="*" value="${roleName}"/>
               <t:choose hiddenName="roleid" hiddenid="id" textname="rolename" 
                url="roleController.do?goRoleSelect"
                name="roleList" icon="icon-search" title="角色列表" isclear="true" isInit="false" left="500" top="150"></t:choose>
      <span class="Validform_checktip"></span>
	</div>
	</fieldset>
</t:formvalid> 
