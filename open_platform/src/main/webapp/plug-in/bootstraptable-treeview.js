...
(function ($) {
    ...
    BootstrapTable.prototype.initData = function () {
        ...
        if (that.options.treeView && this.data.length > 0) {
            ...
            var g=function(child,parentLevel){
    ...
    BootstrapTable.prototype.loadTreeData = function () {
        return this.getData();
    }
    //重写bootstrapTable的initBody方法
    BootstrapTable.prototype.initBody = function (fixedScroll,iconCLick) {
        var that = this,
            html = [],
            data;
        if(iconCLick){
            data = loadNowData(this.$body);
        }else{
            data = this.getData();
        }

        if(data == null || data.length == 0){
            data = [{id:timeId(),parentId:null,isLeaf:true,hidden:false,level:1}];
        }
     ...
     * @param item
     */
    BootstrapTable.prototype.expandTreeNode = function (item)
    {
        var that=this;
        var child = getAllChild(item, that.options.data, that.options.treeId);
        $.each(child, function (i, n) {
    ...
    $.fn.bootstrapTable.methods.push('expandAllTree', 'collapseAllTree','expandTreeNode','loadTreeData');
})(jQuery);